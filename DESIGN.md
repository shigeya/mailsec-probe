...
| **3.0** | 🛠 計画中 | DNSSEC chain validation を `github.com/shigeya/dnsdata-go` (別 module で co-develop) 経由で導入。Phase 1.0 の AD ビット観測モードは `--dnssec-mode ad-only` として残す。詳細は §16 |
| **3.x** | 候補 | 下記 §17 参照 |
...
| 4 | DNSSEC | Phase 1.0–2.5 は AD ビット + DS のみ。Phase 3.0 で `dnsdata-go` 経由の chain validation を導入し、BOGUS / INSECURE を区別 |
...
## 16. Phase 3.0 計画 — DNSSEC validation via dnsdata-go
...
Phase 3.0 では DNSSEC chain validation を **`dnsdata-go` に委譲** する形で導入する。

### 依存先

`github.com/shigeya/dnsdata-go` — 別 module として開発する Go DNS / DNSSEC library。
TypeScript の `dnsdata-js` (wide-cpp-lib 由来) を Go に純血ポートしたもの。
...
### dnsdata-go に対する Requirements

mailsec-probe (= consumer) が dnsdata-go に求める API 契約。
`dnsdata-go` 側 DESIGN.md / Issue にも転記し、co-design の北極星にする。
...
23. デフォルトでファイルシステム書き込みをしない (明示指定時のみ `~/.dnsdata-go/` 等を触る)
...
- 新フラグ `--dnssec-mode {ad-only,validate}` (デフォルト `validate`)
  ...
  - `validate` で dnsdata-go 経由の chain validation
- 新フラグ `--dnssec-doh-providers google,cloudflare,quad9` (デフォルト 3 つ順次)
- 既存 `--dns-server` 指定時は、dnsdata-go の権威 NS 直接問い合わせモードへ自動切替

### golden test への影響

`testdata/domains/<name>/golden.json` の `dnssec` Feature 部が変わるため再生成が必要。
dnsdata-go 側で対応する `testdata/` (chain 検証済みの DNS 応答 fixture) を整備し、
mailsec-probe 側も Mock DNS / Mock DoH で同じ fixture を共有する方針とする。

### スケジュール想定

| 週 | dnsdata-go 側 | mailsec-probe 側 |
|---|---|---|
...
進捗共有とセッション分担は `dnsdata-go` 側 repo の CLAUDE.md / DESIGN.md を参照。
